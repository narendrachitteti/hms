...
const Vitals = () => {
  ...
  const [patientId, setPatientId] = useState(localStorage.getItem('pid')); // Initialize patientId with the value from localStorage
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await axios.post(`http://localhost:3005/patient/Vitals`, vitals);
