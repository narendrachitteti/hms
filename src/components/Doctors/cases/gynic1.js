...
function TrimesterNames() {
  // Create state variables for the names in each trimester
  const [trimesters, setTrimesters] = useState([
    ...
    },
    {
      ...
      names: [
        ...
        { id: 9, value: '', name: 'NST' },
        // Add more names for the 4th trimester as needed
      ],
    },
  ]);

  // Function to update a name when the input changes
  const handleNameChange = (trimester, id, newName) => {
    const updatedTrimesters = trimesters.map((item) => {
      if (item.trimester === trimester) {
        const updatedNames = item.names.map((nameItem) =>
          nameItem.id === id ? { ...nameItem, value: newName } : nameItem
        );
