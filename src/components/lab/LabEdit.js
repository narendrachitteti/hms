...
const LabEdit = () => {
  ...
  const fetchEditData = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/labEditPatientdata/edit');
      ...
      setExistingId(editData.existingId);
      setBloodGroup(editData.bloodGroup);
      setEmail(editData.email);
      setAddress(editData.address);
      setCity(editData.city);
      setArea(editData.area);
      setReferredBy(editData.referredBy);
  ...
  const handleUpdate = async () => {
    ...
    try {
      await axios.put('http://localhost:5000/api//labEditPatientdata/update', updatedData);
